...
<body>
    ...
    <div class=" modal fade-out"  id="infoModal"  >
    ...
    <a href="{{route('homepage')}}" class="logo mr-auto"><img src="assets/img/logo.png" alt="" class="img-fluid"></a>
      ...
      <nav class="nav-menu d-none d-lg-block">
        ...
        <ul>
          ...
          <li><a href="{{route('fee_import')}}"> <i class="fa fa-arrow-circle-left" style="font-size:16px;" >
          </i>&nbsp;back</a></li>
  ...
  <main id="main">
    ...
    <section id="about" class="about">
      <div class="container" data-aos="fade-up">
        ...
        <h2 style="color:  #3084e4">Fee Section  </h2></span>
          <p>List of students with dues</p>
        </div>
...
<div class="card-body">
    ...
    <thead>
      <tr class="text-white text-bold text-capitalize flex-auto" style="background-color: #19293d; font-size:12px">
        ...
        <th class="tablepadding">Semester</th>
        <th class="tablepadding">Balance Remaining</th>
        <th class="tablepadding">Status</th>
        <td class="tablepadding">Created</td>
...
<img src="{{'uploads//'. $item->user_image }}"  class="rounded-circle shadow-lg rounded-circle"  style="padding:3px; width: 12rem; height: auto;"  />
     ...
     <a class="form_image_button" style ="align-items: center" href="{{route('flist.edit',$encrypted)}}"> <span class="glyphicon glyphicon-edit"></span> Add Picture</a>

@endif

        </td>

        <td class="tablepadding">{{ $item->registration_num }}</td>
        <td class="tablepadding">{{ $item->name }}</td>
        <td class="tablepadding">{{ $item->father_name }}</td>
        <td class="tablepadding">{{ $item->semester }}</td>
        <td class="tablepadding">{{ $item->balance }}</td>
        <td class="tablepadding">{{ $item->status }}</td>
        <td class="tablepadding">{{ $item->created_at }}</td>

        <td class="tablepadding">

        <form action="{{route('flist.destroy',$item->id)}}" method="POST" id="submit">
            <?php
            $id = $item->id;
            $encrypted = encrypt($id);
      ?>
  <a  class="tablebuttons" href="{{route('flist.show',$encrypted)}}" data-target="#infoModal" data-toggle="modal"><span class="glyphicon glyphicon-eye-open"></span></a><br>
            ...
            <a class="tablebuttons" href="{{route('flist.edit',$encrypted)}}"> <span class="glyphicon glyphicon-edit"></span></a><br>
                @csrf
                @method('DELETE')
