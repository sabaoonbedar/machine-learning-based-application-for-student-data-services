...
import {AlumniSystemSearch,students_listData,Delete} from '../../../Redux/Actions'
import {
  View,
  Text,
  StyleSheet,
 ...
 class AllStudents extends Component {
  ...
  super(props)
    this.state = {
      ...
      showToast: false,
      text: '',
      data: this.props.students_data,
    
    }

    this.arrayholder = this.props.students_data;
...
renderer=()=>{
  ...
  if(this.props.students_data=='null'){
  
  
  
  }else{
  
  this.props.students_listData();
...
//for the loader when you drage it down
   ...
   this.setState({data:this.props.students_data})
...
return(
  ...
  <FlatList
          ...
          // data={this.props.students_data.sort((a, b) => a.name.localeCompare(b.name))}
...
<Right>      
  ...
  if(this.props.students_data){
    if(this.props.students_data.length){
  setTimeout(() => {
        Actions.refresh({key: Math.random()})
  }, 500);
    }else{
...
const mapStatetoProps = (state)=>{
  ...
  const{alumni_data,refreshing,students_data} = state.Crud_Data;
return{
...
export default connect(mapStatetoProps,{AlumniSystemSearch,students_listData,Delete}) (AllStudents);