...
import {AlumniSystemSearch,female_students,Delete} from '../../../Redux/Actions'
import {
  View,
  Text,
  StyleSheet,
 ...
 class AllStudents extends Component {
  ...
  super(props)
    this.state = {
      ...
      showToast: false,
      text: '',
      data: this.props.students_female_data,
    
    }

    this.arrayholder = this.props.students_female_data;
...
renderer=()=>{
  ...
  if(this.props.students_female_data=='null'){
  
  
  
  }else{
  
  this.props.female_students();
...
//for the loader when you drage it down
   ...
   this.setState({data:this.props.students_female_data})
...
return(
  ...
  <FlatList
          ...
          // data={this.props.students_female_data.sort((a, b) => a.name.localeCompare(b.name))}
...
<Right>      
  ...
  if(this.props.students_female_data){
    if(this.props.students_female_data.length){
  setTimeout(() => {
        Actions.refresh({key: Math.random()})
  }, 500);
    }else{
...
const mapStatetoProps = (state)=>{
  ...
  const{students_female_data} = state.Crud_Data;
return{

search:search,

students_female_data:students_female_data,
...
export default connect(mapStatetoProps,{AlumniSystemSearch,female_students,Delete}) (AllStudents);