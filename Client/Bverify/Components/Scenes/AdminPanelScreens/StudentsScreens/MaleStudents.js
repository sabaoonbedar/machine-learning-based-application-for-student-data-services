...
import {AlumniSystemSearch,male_students,Delete} from '../../../Redux/Actions'
import {
  View,
  Text,
  StyleSheet,
 ...
 class AllStudents extends Component {
  ...
  super(props)
    this.state = {
      ...
      showToast: false,
      text: '',
      data: this.props.students_male_data,
    
    }

    this.arrayholder = this.props.students_male_data;
...
renderer=()=>{
  ...
  if(this.props.students_male_data=='null'){
  
  
  
  }else{
  
  this.props.male_students();
...
//for the loader when you drage it down
   ...
   this.setState({data:this.props.students_male_data})
...
return(
  ...
  <FlatList
          ...
          // data={this.props.students_male_data.sort((a, b) => a.name.localeCompare(b.name))}
...
<Right>      
  ...
  if(this.props.students_male_data){
    if(this.props.students_male_data.length){
  setTimeout(() => {
        Actions.refresh({key: Math.random()})
  }, 500);
    }else{
...
const mapStatetoProps = (state)=>{
  ...
  const{students_male_data} = state.Crud_Data;
return{

search:search,

students_male_data:students_male_data,
...
export default connect(mapStatetoProps,{AlumniSystemSearch,male_students,Delete}) (AllStudents);