...
import {AlumniSystemSearch,bs_students,Delete} from '../../../Redux/Actions'
import {
  View,
  Text,
  StyleSheet,
 ...
 class AllStudents extends Component {
  ...
  super(props)
    this.state = {
      ...
      showToast: false,
      text: '',
      data: this.props.students_bs_data,
    
    }

    this.arrayholder = this.props.students_bs_data;
...
renderer=()=>{
  ...
  if(this.props.students_bs_data=='null'){
  
  
  
  }else{
  
  this.props.bs_students();
...
//for the loader when you drage it down
   ...
   this.setState({data:this.props.students_bs_data})
...
return(
  ...
  <FlatList
          ...
          // data={this.props.students_bs_data.sort((a, b) => a.name.localeCompare(b.name))}
...
<Right>      
  ...
  if(this.props.students_bs_data){
    if(this.props.students_bs_data.length){
  setTimeout(() => {
        Actions.refresh({key: Math.random()})
  }, 500);
    }else{
...
const mapStatetoProps = (state)=>{
  ...
  const{students_bs_data} = state.Crud_Data;
return{

search:search,

students_bs_data:students_bs_data,
...
export default connect(mapStatetoProps,{AlumniSystemSearch,bs_students,Delete}) (AllStudents);