...
import {AlumniSystemSearch,msc_students,Delete} from '../../../Redux/Actions'
import {
  View,
  Text,
  StyleSheet,
 ...
 class AllStudents extends Component {
  ...
  super(props)
    this.state = {
      ...
      showToast: false,
      text: '',
      data: this.props.students_msc_data,
    
    }

    this.arrayholder = this.props.students_msc_data;
...
renderer=()=>{
  ...
  if(this.props.students_msc_data=='null'){
  
  
  
  }else{
  
  this.props.msc_students();
...
//for the loader when you drage it down
   ...
   this.setState({data:this.props.students_msc_data})
...
return(
  ...
  <FlatList
          ...
          // data={this.props.students_msc_data.sort((a, b) => a.name.localeCompare(b.name))}
...
<Right>      
  ...
  if(this.props.students_msc_data){
    if(this.props.students_msc_data.length){
  setTimeout(() => {
        Actions.refresh({key: Math.random()})
  }, 500);
    }else{
...
const mapStatetoProps = (state)=>{
  ...
  const{students_msc_data} = state.Crud_Data;
return{

search:search,

students_msc_data:students_msc_data,
...
export default connect(mapStatetoProps,{AlumniSystemSearch,msc_students,Delete}) (AllStudents);