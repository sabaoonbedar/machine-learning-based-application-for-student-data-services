...
import {AlumniSystemSearch,listData,Delete} from '../../../Redux/Actions'
...
import { connect } from 'react-redux';
 ...
 class DegreeVerifyList extends Component {
  ...
  super(props)
    this.state = {
      ...
      showToast: false,
      text: '',
      data: this.props.alumni_data,
    
    }

    this.arrayholder = this.props.alumni_data;
...
renderer=()=>{
  ...
  if(this.props.alumni_data=='null'){
...
onPress={()=>{
 ...
 Actions.jump('DegreeVerifyShowScreen',
...
<Right>      
  ...
  if(this.props.alumni_data){
    if(this.props.alumni_data.length){
  setTimeout(() => {
        Actions.refresh({key: Math.random()})
  }, 500);
    }else{
...
export default connect(mapStatetoProps,{AlumniSystemSearch,listData,Delete}) (DegreeVerifyList);