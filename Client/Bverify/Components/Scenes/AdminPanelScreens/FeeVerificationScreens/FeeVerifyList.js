...
import {AlumniSystemSearch,fee_students_list,Delete} from '../../../Redux/Actions'
import {
  View,
  Text,
  StyleSheet,
...
import { connect } from 'react-redux';
 ...
 class AlumniSystem extends Component {
  ...
  super(props)
    this.state = {
      ...
      showToast: false,
      text: '',
      data: this.props.students_fee_data,
    
    }

    this.arrayholder = this.props.students_fee_data;
...
renderer=()=>{
  ...
  if(this.props.students_fee_data=='null'){
...
//for the loader when you drage it down
   ...
   this.props.fee_students_list();
   this.setState({data:this.props.students_fee_data})
...
return(
  ...
  <FlatList
          ...
          // data={this.props.students_fee_data.sort((a, b) => a.name.localeCompare(b.name))}
...
<Right>      
  ...
  if(this.props.students_fee_data){
    if(this.props.students_fee_data.length){
  setTimeout(() => {
        Actions.refresh({key: Math.random()})
  }, 500);
    }else{
...
const mapStatetoProps = (state)=>{
  ...
  const{students_fee_data,} = state.Crud_Data;
return{
students_fee_data:students_fee_data,
search:search,
...
export default connect(mapStatetoProps,{AlumniSystemSearch,fee_students_list,Delete}) (AlumniSystem);